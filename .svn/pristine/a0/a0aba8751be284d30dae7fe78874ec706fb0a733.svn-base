...
function rescript(src) {
    const resc = document.createElement('script');
    resc.src = src;
    document.body.appendChild(resc);
}

function jquery() {
    rescript("https://cdn.bootcss.com/jquery/3.2.1/jquery.js")
}

function axios() {
    rescript("https://cdn.bootcdn.net/ajax/libs/axios/1.1.3/axios.js")
}

class Tag {
    constructor() {
        this.csdn = "blog.csdn.net"
        this.netflix = "www.netflix.com"
        this.drown = "192.168.31.208:5173"
    }
}
...
function csdn() {
    window.onload = function () {
        ...
        var ax = $('img');
        var bx = ax.length;
        for (var cx = 0; cx < bx; cx++) {
            var dx = $(ax[cx]);
            var ex = dx.attr('onerror');
            if (ex !== undefined) {
                if (ex.indexOf("setTimeout") !== -1) {
                    dx.remove()
                }
            }
        }
